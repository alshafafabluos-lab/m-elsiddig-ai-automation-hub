...
export default function App() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      let errorMsg = lang === 'ar' ? 'فشل تسجيل الدخول عبر Google: ' : 'Google login failed: ';
      if (err.code === 'auth/popup-blocked') {
        errorMsg += lang === 'ar' ? 'تم حظر النافذة المنبثقة. يرجى السماح بالمنبثقات.' : 'Popup was blocked. Please allow popups.';
      } else if (err.code === 'auth/unauthorized-domain') {
        errorMsg += lang === 'ar' 
          ? `النطاق (${window.location.hostname}) غير مصرح به في Firebase.` 
        ...
        {showLoginModal && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-md glass border border-white/10 rounded-3xl overflow-hidden relative shadow-2xl"
            >
              <button 
                onClick={() => setShowLoginModal(false)}
                className="absolute top-4 right-4 p-2 hover:bg-white/10 rounded-full text-slate-400 transition-all"
              >
                <X size={20} />
              </button>
              ...
              <div className="p-8">
                ...
                <div className="mt-6">
                  <div className="relative mb-6">
                    ...
                    <div className="relative flex justify-center text-xs uppercase"><span className="bg-slate-950 px-2 text-slate-500">{lang === 'ar' ? 'أو' : 'OR'}</span></div>
                  </div>

                  <button 
                    onClick={handleGoogleLogin}
                    className="w-full py-3 glass hover:bg-white/10 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all"
                  >
                    ...
                    {lang === 'ar' ? 'تسجيل الدخول عبر جوجل' : 'Sign in with Google'}
                  </button>
                </div>
              </div>
            </motion.div>
            ...
            <div className="flex justify-between items-center mb-8 border-b border-slate-800 pb-6">
              ...
              <div className="flex items-center gap-4">
                {user ? (
                  ...
                  <button onClick={() => setShowLoginModal(true)} className="px-4 py-2 glass hover:bg-brand-500/10 text-brand-400 text-xs font-bold rounded-xl transition-all">
            ...
            {!isAdmin ? (
               <div className="flex-grow flex items-center justify-center">
                  <div className="text-center max-w-sm">
                    ...
                    {!user && (
                      <button onClick={() => setShowLoginModal(true)} className="w-full py-4 bg-brand-600 text-white rounded-2xl font-bold shadow-lg">
                        {lang === 'ar' ? 'تسجيل الدخول للنظام' : 'Access Control Login'}
                      </button>
                    )}
                  </div>
               </div>
